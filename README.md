...
<p>Full-stack monorepo for real-time infrastructure monitoring across a multi-site deployment spanning 50 hectares. A modular adapter registry ingests events from any webhook-based source, new sources requiring only a new adapter class — delivers Telegram notifications with interactive acknowledgement buttons. Features a bespoke Next.js dashboard with live event feeds, UPS status charts, network vitals, ISP metrics, and a device fleet grid. Includes a conversational AI SysAdmin agent with tool-calling that answers natural language queries about cameras, UPS state, and network health.</p>
<p>
